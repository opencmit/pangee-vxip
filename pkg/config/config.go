...
func (l *PortRange) Equal(r *PortRange) bool {
	if l == nil || r == nil {
		return false
	}
	if l.Min != r.Min {
...
type Global struct {
	...
	AvailablePortRange PortRange `mapstructure:"available-port-range" yaml:"available-port-range"`
...
func (l *Global) Equal(r *Global) bool {
	...
	if !l.AvailablePortRange.Equal(&r.AvailablePortRange) {
		return false
	}
	if l.Group != r.Group {
		return false
...
type Balancer struct {
	...
	UseXdp               bool      `mapstructure:"use-xdp" yaml:"use-xdp,omitempty"`
