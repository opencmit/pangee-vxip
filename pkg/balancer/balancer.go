...
func NewBalancer(conf *config.ConfigSet) (*Balancer, error) {
	...
	if len(conf.Global.NatFilePath) > 0 {
		b.nat, err = bpf.NewNAT(&conf.Global)
		if err != nil {
	...
	if len(conf.Global.FNatFilePath) > 0 {
		b.fnat, err = bpf.NewFNAT(&conf.Global, &conf.FNat)
		if err != nil {
...
func (b *Balancer) GetServices() ([]comm.SrvKey, []comm.SrvVal) {
	keys := make([]comm.SrvKey, 0, defaultSrvLen)
	vals := make([]comm.SrvVal, 0, defaultSrvLen)
	b.services.Range(func(k, v interface{}) bool {
		key, ok := k.(comm.SrvKey)
		if !ok {
			return true
		}
		val, ok := v.(*comm.SrvVal)
		if !ok {
			return true
		}
...
func (b *Balancer) attach(ip string, mode comm.BpfMode) error {
	...
	b.attached.Store(ip, &attachInfo{mode: mode})
	return nil
}

func (b *Balancer) detach(ip string, mode comm.BpfMode) error {
	var err error
	if v, ok := b.attached.Load(ip); !ok {
		return nil
