...
#define MAX_COLLISION_TIMES 4
#define STRATEGY_SHARED     0
#define STRATEGY_EXCLUSIVE  1

enum warp_fail_code {
    WARP_CODE_OK = 0,
    WARP_CODE_NOT_FOUND_SERVER,
    WARP_CODE_INVALID_STRATEGY,
    WARP_CODE_INVALID_PORT,
    WARP_CODE_SAVE_FSESSION_FAILED,
    WARP_CODE_SAVE_RSESSION_FAILED,
    WARP_CODE_SWAP_FAILED,
    WARP_CODE_DEC_TTL_FAILED,
    WARP_CODE_REDIRECT_FAILED,
    WARP_CODE_MAX,
};
...
{
    __u32 *inner_fd = bpf_map_lookup_elem(&warp_map_fnat_real_server, key);
    if (!inner_fd) {
        return 0;
    }
...
static __always_inline __be16 select_shared_port(struct warp_sockaddr *src, struct warp_sockaddr *dst, __u16 min_port, __u16 max_port)
{
    struct warp_connection conn;
    conn.saddr = *src;
    conn.daddr = *dst;
    __u16 port = bpf_ntohs(conn.daddr.port);
    struct warp_redirect *exist = 0;
    if (port >= min_port && port <= max_port) {
        exist = bpf_map_lookup_elem(&warp_map_fnat_connection, &conn);
        if (!exist) {
            return conn.daddr.port;
        }
    }

    __u16 range = max_port - min_port + 1;
    for (int i = 0; i < MAX_COLLISION_TIMES; i++) {
        __u32 rand = bpf_get_prandom_u32();
        port = (__u16)((rand + port) % range) + min_port;
...
{
    __u32 *inner_fd = bpf_map_lookup_elem(&warp_map_fnat_port, &cpu);
    if (!inner_fd) {
        return 0;
    }
...
static __always_inline void release_port(__be16 port)
{
    ...
    bpf_map_update_elem(&warp_map_fnat_release_port, &key, &val, BPF_ANY);
}
...
static __always_inline enum warp_action get_or_gen_conn(struct warp_context *ctx)
{
    ...
    struct warp_redirect *look_fwd = bpf_map_lookup_elem(&warp_map_fnat_connection, &fwd_key);
    if (look_fwd) {
        ctx->forward       = *look_fwd;
        look_fwd->ts       = bpf_ktime_get_ns();
        look_fwd->is_local = WARP_SET;
        return WARP_ACT_OK;
    }

    struct warp_service *srv = bpf_map_lookup_elem(&warp_map_fnat_service, &fwd_key.daddr);
    ...
    if (!rs) {
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_NOT_FOUND_SERVER;
        bpf_perf_event_output(ctx->stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
    ...
    ctx->forward.is_local            = WARP_SET;
    ctx->forward.is_local_port       = WARP_UNSET; // this conn will not use exclusive port
    ctx->forward.positive            = WARP_SET;
    ctx->forward.ts                  = bpf_ktime_get_ns();
    if (srv->strategy == STRATEGY_SHARED) {
        ctx->forward.redirect.saddr.port = select_shared_port(&ctx->forward.redirect.daddr,
            &ctx->forward.redirect.saddr, srv->min_port, srv->max_port);
    } else if (srv->strategy == STRATEGY_EXCLUSIVE) {
        ctx->forward.redirect.saddr.port = select_exclusive_port(ctx->cpu);
    } else {
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_INVALID_STRATEGY;
        bpf_perf_event_output(ctx->stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
        return WARP_ACT_PASS;
    }
    ...
    if (!ctx->forward.redirect.saddr.port) {
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_INVALID_PORT;
        bpf_perf_event_output(ctx->stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
        return WARP_ACT_PASS;
    }
    ...
    reverse.is_local        = WARP_SET;
    reverse.is_local_port   = WARP_SET; // this conn will use exclusive port
    reverse.ts              = ctx->forward.ts;

    long update_ret = bpf_map_update_elem(&warp_map_fnat_connection, &fwd_key, &ctx->forward, BPF_NOEXIST);
    if (update_ret) {
        if (srv->strategy == STRATEGY_EXCLUSIVE) {
            release_port(ctx->forward.redirect.saddr.port);
        }
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_SAVE_FSESSION_FAILED;
        bpf_perf_event_output(ctx->stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
        return WARP_ACT_DROP;
    }
    update_ret = bpf_map_update_elem(&warp_map_fnat_connection, &rvs_key, &reverse, BPF_NOEXIST);
    if (update_ret) {
        bpf_map_delete_elem(&warp_map_fnat_connection, &fwd_key);
        if (srv->strategy == STRATEGY_EXCLUSIVE) {
            release_port(ctx->forward.redirect.saddr.port);
        }
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_SAVE_RSESSION_FAILED;
        bpf_perf_event_output(ctx->stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
        return WARP_ACT_DROP;
    }
...
SEC("xdp_fnat")
int warp_xdp_fnat(struct xdp_md *xdp_ctx)
{
    ...
    if (act != WARP_ACT_OK) {
        if (act == WARP_ACT_UNREACH) {
            ...
            bpf_perf_event_output(ctx.stack_ctx, &warp_map_fnat_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
        }
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_SWAP_FAILED;
        bpf_perf_event_output(ctx.stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
        return warp_xdp_act(act);
    }
    ...
    if (act != WARP_ACT_OK) {
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_DEC_TTL_FAILED;
        bpf_perf_event_output(ctx.stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
        return warp_xdp_act(act);
    }
...
SEC("tc_fnat")
int warp_tc_fnat(struct __sk_buff *skb)
{
    struct warp_context ctx;
    __builtin_memset(&ctx, 0, sizeof(ctx));
...
*/
    ...
    int ret = bpf_redirect(ctx.forward.redirect_if_idx, 0);
    if (ret != TC_ACT_REDIRECT) {
        struct warp_fail_event ev;
        ev.code = (__u32)WARP_CODE_REDIRECT_FAILED;
        bpf_perf_event_output(ctx.stack_ctx, &warp_map_fail_event, BPF_F_CURRENT_CPU, &ev, sizeof(ev));
    }
    return ret;
}
