#ifndef __WARP_HELPERS_H__
#define __WARP_HELPERS_H__

#include "warp_structs.h"
#include "warp_maps.h"
#include "warp_csum_helpers.h"

#define WARP_IPV6_HDR_LEN       40
#define WARP_IPV6_FLOWINFO_MASK __cpu_to_be32(0x0FFFFFFF)

static __always_inline enum warp_action warp_parse_ctx(struct warp_context *ctx)
{
    // L2
    ...
    if (eth_p_ip == ETH_P_IP) {
        ...
        ctx->org_saddr.addr6 = *(struct warp_ipv6*)(&ipv6h->saddr);
        ctx->org_daddr.addr6 = *(struct warp_ipv6*)(&ipv6h->daddr);
    } else {
...
static __always_inline enum warp_action warp_swap_addr(struct warp_context *ctx)
{
    ...
    if (ctx->is_ipv6) {
        ...
        fib_params.flowinfo    = *(__be32*)ipv6h & WARP_IPV6_FLOWINFO_MASK;
...
#define WARP_MAX_IP_HDR_LEN 40
#define WARP_IP_TOA_OP      254

static __always_inline enum warp_action warp_tc_extend_toa(struct warp_context *ctx)
{
    ...
    if (ctx->l4h + tcphdr_len != ctx->end || tcphdr_len > WARP_MAX_IP_HDR_LEN) {
        return WARP_ACT_OK;
    }
    ...
    __u16 old_doff[2] = {0};
    __u16 new_doff[2] = {0};
    iph->tot_len = bpf_htons(iph->ihl*4 + tcphdr_len + sizeof(struct warp_toa));
    old_doff[0]  = *(((__u16*)&tcph->ack_seq) + 2);
    tcph->doff   = tcph->doff + sizeof(struct warp_toa) / 4;
    new_doff[0]  = *(((__u16*)&tcph->ack_seq) + 2);
    long ret     = bpf_skb_change_tail(ctx->stack_ctx, old_pkglen + sizeof(struct warp_toa), 0);
    if (ret != 0) {
        return WARP_ACT_DROP;
    }
    ...
    enum warp_action rc = warp_parse_ctx(ctx);
    if (rc != WARP_ACT_OK) {
        return rc;
    }
    ...
    struct warp_toa toa;
    toa.opcode = WARP_IP_TOA_OP;
    toa.opsize = 8;
    toa.port   = ctx->org_saddr.port;
    toa.ip     = ctx->org_saddr.addr4.addr;
    ret = bpf_skb_store_bytes(ctx->stack_ctx, old_pkglen, &toa, sizeof(struct warp_toa), 0);
    if (ret != 0) {
        return WARP_ACT_DROP;
    }
    ...
    payload      += sizeof(struct warp_toa);
    new_doff[1]   = bpf_htons(payload);
    warp_update_csum(&cs, *((__u32*)old_doff), *((__u32*)new_doff));
    warp_update_csum(&cs, 0, *((__be32*)&toa));
    warp_update_csum(&cs, 0, toa.ip);
    tcph->check   = cs;
...
static __always_inline enum warp_action warp_dec_ttl(struct warp_context *ctx)
{
    if (ctx->is_ipv6) {
...
static __always_inline int warp_xdp_act(enum warp_action act)
{
    switch (act) {
        case WARP_ACT_OK:
            return XDP_ABORTED;
        case WARP_ACT_DROP:
        case WARP_ACT_UNREACH:
            return XDP_DROP;
        case WARP_ACT_REDIRECT:
            return XDP_REDIRECT;
        default:
            return XDP_PASS;
    }
...
static __always_inline int warp_tc_act(enum warp_action act)
{
    switch (act) {
        case WARP_ACT_DROP:
        case WARP_ACT_UNREACH:
