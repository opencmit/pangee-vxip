...
type natElf struct {
	Prog         *ebpf.Program `ebpf:"warp_xdp_nat"`
	Service      *ebpf.Map     `ebpf:"warp_map_nat_service"`
	InRealServer *ebpf.Map     `ebpf:"warp_map_in_real_server"`
	RealServer   *ebpf.Map     `ebpf:"warp_map_nat_real_server"`
	Connection   *ebpf.Map     `ebpf:"warp_map_nat_connection"`
	Event        *ebpf.Map     `ebpf:"warp_map_nat_event"`
}
...
func NewNAT(cfg *config.Global) (comm.Service, error) {
	s := &nat{timeout: cfg.ConnTimeout}
	...
	if s.spec, err = ebpf.LoadCollectionSpec(cfg.NatFilePath); err != nil {
		log.Errorf("load collection spec failed: %v", err)
		return nil, err
	}
	_, ok := s.spec.Maps[natRealServerMapName]
	if !ok {
		log.Errorf("get spec failed: map object[%s] missing", natRealServerMapName)
		return nil, fmt.Errorf("%s object missing", natRealServerMapName)
	}
	if s.inRealServerSpec, ok = s.spec.Maps[inRealServerMapName]; !ok {
	...
	s.spec.Maps[natRealServerMapName].InnerMap = s.spec.Maps[inRealServerMapName]
	s.spec.Maps[natRealServerMapName].Extra = nil

	if err = s.spec.LoadAndAssign(&s.elf, nil); err != nil {
...
func (s *nat) Attach(ip string) error {
	ifIdx, err := intf.GetIfIdxByIp(ip)
	if err != nil {
		return err
	}

	return comm.AttachLink(ifIdx, s.elf.Prog.FD())
}

func (s *nat) Detach(ip string) error {
	ifIdx, err := intf.GetIfIdxByIp(ip)
	if err != nil {
		return err
	}
