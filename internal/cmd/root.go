...
func NewRootCmd() *cobra.Command {
	...
	rootCmd.PersistentFlags().IntVarP(&globalOpts.Port, "port", "p", comm.DefailtApiPort, "warp deamon service api port")
	rootCmd.AddCommand([]*cobra.Command{ /*newNatCmd(),*/ newFNatCmd(), newSaveCmd(), newServiceCmd()}...)
	return rootCmd
}

func newClient(ctx context.Context) (api.WarpApiClient, context.CancelFunc, error) {
	...
	return api.NewWarpApiClient(conn), cancel, nil
}
